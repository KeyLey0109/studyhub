...
class LoadPostsEvent extends PostEvent { final bool refresh; LoadPostsEvent({this.refresh = false}); }
class CreatePostEvent extends PostEvent {
  ...
  CreatePostEvent({
    ...
    this.authorAvatar, this.content,
    this.mediaUrls = const [], this.mediaTypes = const [],
  });
}
class ReactToPostEvent extends PostEvent {
  final String postId, userId; final ReactionType? reactionType;
  ReactToPostEvent({required this.postId, required this.userId, this.reactionType});
}
class AddCommentEvent extends PostEvent {
  final String postId, authorId, authorName, content;
  final String? authorAvatar, parentId;
  AddCommentEvent({
    required this.postId, required this.authorId, required this.authorName,
    this.authorAvatar, required this.content, this.parentId,
  });
}
class LikeCommentEvent extends PostEvent {
  final String postId, commentId, userId;
  LikeCommentEvent({required this.postId, required this.commentId, required this.userId});
}
...
class PostLoaded extends PostState {
  ...
  @override List<Object?> get props => [posts, hasMore];
}
...
class PostBloc extends Bloc<PostEvent, PostState> {
  ...
  Future<void> _onLoad(LoadPostsEvent event, Emitter<PostState> emit) async {
    ...
    try {
      ...
      final current = state is PostLoaded && !event.refresh ? (state as PostLoaded).posts : <PostEntity>[];
      _page++;
      emit(PostLoaded(posts: [...current, ...posts], hasMore: posts.length >= 10));
    } catch (e) { emit(PostError(e.toString())); }
  }

  Future<void> _onCreate(CreatePostEvent event, Emitter<PostState> emit) async {
    try {
      final post = await repository.createPost(
        authorId: event.authorId, authorName: event.authorName,
        authorAvatar: event.authorAvatar, content: event.content,
        mediaUrls: event.mediaUrls, mediaTypes: event.mediaTypes,
      );
      final current = state is PostLoaded ? (state as PostLoaded).posts : <PostEntity>[];
  ...
  Future<void> _onReact(ReactToPostEvent event, Emitter<PostState> emit) async {
    try {
      final updated = await repository.reactToPost(event.postId, event.userId, event.reactionType);
      _updatePost(updated, emit);
    } catch (_) {}
  }

  Future<void> _onComment(AddCommentEvent event, Emitter<PostState> emit) async {
    try {
      final updated = await repository.addComment(
        postId: event.postId, authorId: event.authorId,
        authorName: event.authorName, authorAvatar: event.authorAvatar,
        content: event.content, parentId: event.parentId,
      );
      _updatePost(updated, emit);
    } catch (_) {}
  }

  Future<void> _onLikeComment(LikeCommentEvent event, Emitter<PostState> emit) async {
    try {
      final updated = await repository.likeComment(event.postId, event.commentId, event.userId);
      _updatePost(updated, emit);
    } catch (_) {}
  }

  Future<void> _onSearch(SearchPostsEvent event, Emitter<PostState> emit) async {
    // handled in SearchBloc
  }

  void _updatePost(PostEntity updated, Emitter<PostState> emit) {
    if (state is PostLoaded) {
      final posts = (state as PostLoaded).posts.map((p) => p.id == updated.id ? updated : p).toList();
