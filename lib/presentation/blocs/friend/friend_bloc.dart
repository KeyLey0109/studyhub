...
class LoadFriendsEvent extends FriendEvent { final String userId; LoadFriendsEvent(this.userId); }
class LoadFriendRequestsEvent extends FriendEvent { final String userId; LoadFriendRequestsEvent(this.userId); }
class LoadSuggestionsEvent extends FriendEvent { final String userId; LoadSuggestionsEvent(this.userId); }
...
class RespondFriendRequestEvent extends FriendEvent {
  final String fromId, toId; final bool accept;
  RespondFriendRequestEvent({required this.fromId, required this.toId, required this.accept});
}
...
class FriendLoaded extends FriendState {
  ...
  FriendLoaded({this.friends = const [], this.requests = const [], this.suggestions = const []});
  FriendLoaded copyWith({List<UserEntity>? friends, List<UserEntity>? requests, List<UserEntity>? suggestions}) =>
    FriendLoaded(friends: friends ?? this.friends, requests: requests ?? this.requests, suggestions: suggestions ?? this.suggestions);
  @override List<Object?> get props => [friends, requests, suggestions];
}
class FriendError extends FriendState { final String message; FriendError(this.message); }
...
class FriendBloc extends Bloc<FriendEvent, FriendState> {
  ...
  Future<void> _onLoadFriends(LoadFriendsEvent event, Emitter<FriendState> emit) async {
    ...
    try {
      ...
      emit(FriendLoaded(friends: friends, requests: requests, suggestions: suggestions));
    } catch (e) { emit(FriendError(e.toString())); }
  }

  Future<void> _onLoadRequests(LoadFriendRequestsEvent event, Emitter<FriendState> emit) async {
    try {
      final requests = await getFriendRequestsUseCase(event.userId);
      final current = state is FriendLoaded ? state as FriendLoaded : FriendLoaded();
  ...
  Future<void> _onSendRequest(SendFriendRequestEvent event, Emitter<FriendState> emit) async {
    try {
      ...
      final suggestions = current.suggestions.where((u) => u.id != event.toId).toList();
      emit(current.copyWith(suggestions: suggestions));
    } catch (e) { emit(FriendError(e.toString())); }
  }

  Future<void> _onRespond(RespondFriendRequestEvent event, Emitter<FriendState> emit) async {
    try {
      await acceptRequestUseCase(fromId: event.fromId, toId: event.toId, accept: event.accept);
      final current = state is FriendLoaded ? state as FriendLoaded : FriendLoaded();
      final requests = current.requests.where((u) => u.id != event.fromId).toList();
      final friends = event.accept
          ? [...current.friends, ...current.requests.where((u) => u.id == event.fromId)]
          : current.friends;
